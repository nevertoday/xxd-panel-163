...
서로 다른 원본에서 각각 한 번씩 생성한 Panel 163 샘플 8장입니다. 짧은 영어 문구를 사용했습니다. 가로 1536×864는 좌우 각 50%, 세로 1152×1536는 상하 각 50%입니다.

작은 중앙 피사체, 단색 면과 넓은 여백을 확인했습니다. 일부 색은 탁하고 자동차의 세부 묘사가 다소 많습니다. 실제 결과를 표시합니다.
...
중앙에 작은 단색 면 스탬프를 두고 주변에 아주 큰 여백을 남깁니다. 복잡한 질감이나 입자 해칭은 쓰지 않습니다.
...
- `design-only`: 전체 캔버스에 Panel 163의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-163`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-163 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-163-prompt.en.md)와 [중국어](references/xxd-panel-163-prompt.zh-CN.md)를 확인하세요.
